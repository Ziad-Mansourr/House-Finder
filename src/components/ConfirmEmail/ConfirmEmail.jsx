...
export default function ConfirmEmail() {
    ...
    const email = location.state?.email || ""; // ناخد الايميل من Signup
    ...
    async function resendOtp() {
        try {
            await axios.patch("/api/v1/auth/re-send-otp", {
                email
    ...
    return (
        <section className="min-h-screen bg-sky-200 flex items-center justify-center">
            <div className="bg-white p-6 rounded-xl shadow-lg w-[320px]">
                <h2 className="text-center text-xl font-semibold mb-4">Confirm Your Email</h2>
                ...
                <p className="text-center mt-4 text-sm">
                    Didn't receive OTP?{" "}
                    <button onClick={resendOtp} className="text-sky-500 font-medium">
                        Resend
                    </button>
                </p>
            </div>
